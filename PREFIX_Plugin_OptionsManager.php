...
class PREFIX_Plugin_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wp-media-cdn'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'wp-media-cdn'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'wp-media-cdn'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'wp-media-cdn'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'wp-media-cdn');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'wp-media-cdn'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'wp-media-cdn');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'wp-media-cdn'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'wp-media-cdn') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'wp-media-cdn');
            case 'false':
                return __('false', 'wp-media-cdn');

            case 'Administrator':
                return __('Administrator', 'wp-media-cdn');
            case 'Editor':
                return __('Editor', 'wp-media-cdn');
            case 'Author':
                return __('Author', 'wp-media-cdn');
            case 'Contributor':
                return __('Contributor', 'wp-media-cdn');
            case 'Subscriber':
                return __('Subscriber', 'wp-media-cdn');
            case 'Anyone':
                return __('Anyone', 'wp-media-cdn');
        }
        return $optionValue;
    }
